...
export default function Module2Page() {
  ...
  useEffect(() => {
    ...
    loadModuleQuestions(2, gender).then(qs => { setQuestions(qs); setLoading(false); })
      .catch(() => setLoading(false));
  }, [router]);

  if (loading) return <div className="min-h-screen flex items-center justify-center text-secondary">Loading questions...</div>;

  return <ModulePage moduleNumber={2} title="Who You Are" questions={questions} nextPath="/assessment" />;
}
