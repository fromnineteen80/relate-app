...
export default function Module4Page() {
  ...
  useEffect(() => {
    ...
    loadModuleQuestions(4, gender).then(qs => { setQuestions(qs); setLoading(false); })
      .catch(() => setLoading(false));
  }, [router]);

  if (loading) return <div className="min-h-screen flex items-center justify-center text-secondary">Loading questions...</div>;

  return <ModulePage moduleNumber={4} title="When Things Get Hard" questions={questions} nextPath="/assessment/processing" />;
}
