...
export default function Module3Page() {
  ...
  useEffect(() => {
    ...
    loadModuleQuestions(3, gender).then(qs => { setQuestions(qs); setLoading(false); })
      .catch(() => setLoading(false));
  }, [router]);

  if (loading) return <div className="min-h-screen flex items-center justify-center text-secondary">Loading questions...</div>;

  return <ModulePage moduleNumber={3} title="How You Connect" questions={questions} nextPath="/assessment" />;
}
